...
const Dialog: React.FC<{ onClose: () => void }> = (props) => {
  ...
  return ReactDOM.createPortal(
    <div className="bui-citySelect-dialog">
      <header>
      ...
      <CurrentLocation />
      <h2>全部城市</h2>
      <ol className="bui-citySelect-index">
        {indexList.map((a) => (
          <li key={a}>{a}</li>
        ))}
      </ol>
      ...
      {cityList.map(([letter, list]) => {
        return (
          <div key={letter} className="bui-citySelect-citySection">
            <h4 data-letter={letter}>{letter}</h4>
            {list.map((city) => (
              <div className="bui-citySelect-cityName" key={city} onClick={() => onClick(city)}>
                {city}
              </div>
            ))}
          </div>
